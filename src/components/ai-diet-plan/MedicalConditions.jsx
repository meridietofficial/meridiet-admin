...
import { LuStethoscope, LuPlus, LuPencil, LuTrash2, LuRotateCcw, LuHash, LuAlignLeft, LuArrowUpDown, LuPin } from "react-icons/lu";
import toast from "react-hot-toast";
import medicalConditionsService from "../../services/medicalConditionsService";

const EMPTY_FORM = { condition_key: "", label: "", description: "", display_order: "" };
...
export default function MedicalConditions() {
  ...
  const fetchConditions = () => {
    setLoading(true);
    medicalConditionsService.list()
      .then((res) => setConditions(res?.data?.data || []))
      .catch((err) => toast.error(err?.response?.data?.message || "Failed to fetch medical conditions."))
  ...
  const stats = {
    ...
    active: conditions.filter((c) => c.is_active === 1 && c.is_deleted === 0).length,
    inactive: conditions.filter((c) => c.is_active === 0 && c.is_deleted === 0).length,
    deleted: conditions.filter((c) => c.is_deleted === 1).length,
  };
  ...
  const handleSave = () => {
    if (!form.condition_key.trim()) { toast.error("Condition key is required."); return; }
    if (!form.label.trim()) { toast.error("Label is required."); return; }

    const payload = {
      condition_key: form.condition_key.trim().toLowerCase(),
    ...
    setSaving(true);
    const call = editing
      ? medicalConditionsService.update(editing.id, payload)
      : medicalConditionsService.create(payload);

    call
      .then(() => {
        toast.success(editing ? "Medical condition updated!" : "Medical condition created!");
        setShowForm(false);
        fetchConditions();
      })
  ...
  const openDelete = (condition) => { setDeleteTarget(condition); setShowDelete(true); };

  const handleDelete = () => {
    if (!deleteTarget) return;
    setDeleting(true);
    medicalConditionsService.remove(deleteTarget.id)
      .then(() => {
  ...
  const handleRestore = (condition) => {
    medicalConditionsService.update(condition.id, { is_deleted: 0, is_active: 1 })
      .then(() => { toast.success("Medical condition restored."); fetchConditions(); })
      .catch((err) => toast.error(err?.response?.data?.message || "Failed to restore."));
  };

  const handleToggleActive = (condition) => {
    const newActive = condition.is_active === 1 ? 0 : 1;
    medicalConditionsService.update(condition.id, { is_active: newActive })
      .then(() => { toast.success(newActive ? "Condition activated." : "Condition deactivated."); fetchConditions(); })
    ...
    <div style={{ padding: "4px 0" }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
            ...
            <h2 style={{ fontWeight: 800, margin: 0, fontSize: "20px" }}>
              <span style={{ color: "#111827" }}>MEDICAL CONDITIONS</span>
              <span style={{ color: "#1E8E3E" }}> LIST</span>
            </h2>
          </div>
          <p style={{ color: "#999", marginBottom: 0, paddingLeft: "14px", fontSize: "13px" }}>Manage medical conditions used in AI diet plan generation.</p>
        </div>

        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
          <StatBox label="Total Conditions" value={stats.total} color="#6366f1" bg="#f5f3ff" />
      ...
      <div style={{ background: "#fff", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            ...
            <p style={{ marginTop: "16px", color: "#999", fontSize: "14px", fontWeight: 500 }}>Loading medical conditions...</p>
          </div>
        ) : filtered.length > 0 ? (
          <div style={{ overflowX: "auto", borderRadius: "12px", border: "1px solid #edf1ee" }}>
            <Table className="table mb-0" style={{ borderCollapse: "collapse", minWidth: "700px" }}>
              <thead>
                <tr>
                  {["S.No", "Label", "Condition Key", "Description", "Order", "Status", "Actions"].map((col) => (
              ...
              <tbody>
                {filtered.map((c, i) => (
                  <tr key={c.id}
                    ...
                    <td style={{ padding: "12px 16px", verticalAlign: "middle" }}>
                      <span style={{ fontFamily: "monospace", fontSize: "12px", color: "#6366f1", background: "#f5f3ff", padding: "3px 8px", borderRadius: "6px", border: "1px solid #e0e7ff" }}>{c.condition_key}</span>
                    </td>
          ...
          <div style={{ textAlign: "center", padding: "60px 20px", borderRadius: "12px", border: "1px solid #edf1ee" }}>
            <div style={{ width: "60px", height: "60px", borderRadius: "50%", background: "#f0fdf4", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
              <LuStethoscope size={26} color="#1E8E3E" />
            </div>
            <h5 style={{ fontWeight: 700, color: "#333", marginBottom: "8px" }}>No Medical Conditions Found</h5>
            <p style={{ fontSize: "14px", color: "#999", marginBottom: "16px" }}>
              {filter !== "all" ? `No ${filter} conditions found.` : "No medical conditions have been added yet."}
            </p>
            {filter === "all" && (
        ...
        {filtered.length > 0 && (
          <p style={{ margin: "12px 0 0", fontSize: "12px", color: "#9ca3af" }}>
            Showing <strong style={{ color: "#111827" }}>{filtered.length}</strong> of <strong style={{ color: "#111827" }}>{conditions.length}</strong> conditions
          </p>
        )}
      </div>
      ...
      <Modal show={showForm} onHide={() => !saving && setShowForm(false)} size="md" centered>
        <Modal.Header closeButton style={{ background: "linear-gradient(135deg, #1E8E3E 0%, #0f5c26 100%)", color: "#fff", borderBottom: "none", padding: "1.2rem 1.6rem" }}>
          <Modal.Title style={{ fontWeight: 800, fontSize: "1rem", display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "rgba(255,255,255,0.18)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <LuStethoscope size={16} color="#fff" />
            </div>
            {editing ? `Edit — ${editing.label}` : "Add Medical Condition"}
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body style={{ padding: "24px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div>
              <FieldLabel label="Condition Key" required />
              <StyledInput icon={LuHash} placeholder="e.g. diabetes" value={form.condition_key}
                onChange={(e) => setField("condition_key", e.target.value.toLowerCase().replace(/\s+/g, "_"))}
                disabled={!!editing}
                style={{ background: editing ? "#f9fafb" : "#fff", cursor: editing ? "not-allowed" : "text" }} />
              <p style={{ margin: "4px 0 0 2px", fontSize: "11px", color: "#9ca3af" }}>
                {editing ? "Condition key cannot be changed after creation." : "Unique internal key — auto lowercased, spaces → underscores."}
              </p>
            </div>

            <div>
              <FieldLabel label="Label" required />
              <StyledInput icon={LuAlignLeft} placeholder="e.g. Diabetes" value={form.label}
                onChange={(e) => setField("label", e.target.value)} />
            </div>

            <div>
              <FieldLabel label="Description" />
              <StyledTextarea rows={3} placeholder="e.g. Type 1 or Type 2 diabetes requiring blood sugar management"
                value={form.description} onChange={(e) => setField("description", e.target.value)} />
            </div>

            <div>
              <FieldLabel label="Display Order" />
              {editing?.condition_key === "other" ? (
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{
                ...
                {saving
                  ...
                  : <><LuStethoscope size={14} /> {editing ? "Update Condition" : "Create Condition"}</>
                }
              </button>
            </div>
          </div>
      ...
      <Modal show={showDelete} onHide={() => !deleting && setShowDelete(false)} size="sm" centered>
        <Modal.Body style={{ padding: "32px 24px", textAlign: "center" }}>
          ...
          <h5 style={{ fontWeight: 700, marginBottom: "8px" }}>Delete Medical Condition?</h5>
